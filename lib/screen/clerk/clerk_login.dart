...
import 'package:webcam_app/utils/log.dart';
...
class _ClerkLoginScreenState extends State<ClerkLoginScreen> {
                                      ...
                                      children: <Widget>[
                                        ...
                                        RequestBtn(
                                          ...
                                          onPress: () async {
                                            ...
                                            List<Clerk> clerk = await ClerkDao
                                                .instance
                                                .readAllNotes();
                                            try {
  ...
  Future _getIPAddress() async {
    String result;
    try {
      var response = await http.get(Uri.parse(Config.WORKTYPE_URL)); //2
      Map<String, dynamic> json = jsonDecode(response.body);
      List<dynamic> data = json["data"];
      for (int i = 0; i < data.length; i++) {
        _items
            .add({'value': data[i]['workType'], 'label': data[i]['workName']});
      }
      print(data);
      print(_items);
    } catch (exception) {
      result = exception.toString();
      print("I am exception" + result);
    }
  }
}
